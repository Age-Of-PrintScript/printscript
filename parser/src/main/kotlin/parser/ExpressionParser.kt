...
internal class ExpressionParser {
    ...
    private fun createExpressionTree(tokens: List<Token>): Either<SyntaxError, ParsedResult<Expression>> =
        when (val term = parseTerm(tokens, 0)) {
    ...
    private fun parseTerm(
        ...
        when (val factor = parseFactor(tokens, position)) {
    ...
    private fun parseParenthesisExpression(
        ...
        when (val expression = createExpressionTree(tokens.subList(position, tokens.size))) {
    ...
    private fun checkClosingParenthesis(
        ...
        when (tokens.getOrNull(position)?.type) {
